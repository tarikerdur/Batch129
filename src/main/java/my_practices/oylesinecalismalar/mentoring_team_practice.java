...
//        output [1,3,5,6,7]
        ...
        Integer []arrayyyyyy = {1,3,5,3,5,6,1,7};
        myList8.addAll(List.of(arrayyyyyy));
        ...
        int [][] arrrrrrr = {{10,20,30},{4},{6,7,20}};
        int [] brreeee = new int[arrrrrrr.length];

        for (int i = 0; i < arrrrrrr.length; i++) {
            for (int j = 0; j <arrrrrrr[i].length ; j++) {
                brreeee[i] += arrrrrrr[i][j];
            }
        }
        for (int w : brreeee){
            if (w==brreeee[0]){
